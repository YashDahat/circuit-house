...
export const getOrdersByCustomerEmail = async (customerEmail: string): Promise<OrderResponse[]> => {
  const response = await apiClient.get<OrderResponse[]>(`/api/v1/orders/customer/${customerEmail}`);
  return response.data;
};
