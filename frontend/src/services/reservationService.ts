...
import type { CreateReservationRequest, ReservationDto, UpdateReservationStatusRequest } from '@/types/reservation';
...
export const getReservationsByStatus = async (status: ReservationStatus): Promise<ReservationDto[]> => {
  const response = await apiClient.get<ReservationDto[]>(`/api/v1/admin/reservations/status/${status}`);
  return response.data;
};

export const updateReservationStatus = async (id: string, request: UpdateReservationStatusRequest): Promise<ReservationDto> => {
  const response = await apiClient.put<ReservationDto>(`/api/v1/admin/reservations/${id}/status`, request);
  return response.data;
};
